...
trait ProductFormats extends ProductFormatsInstances {
  ...
  def jsonFormat0[T](construct: () => T): RootJsonFormat[T] =
    new RootJsonFormat[T] {
      def write(p: T) = JObject.empty
      def read(value: JValue) = value match {
        case JObject(_) => construct()
  ...
  protected def fromField[T](value: JValue, fieldName: String)
                            (implicit reader: JsonReader[T]) = value match {
    case x: JObject if
    reader.isInstanceOf[OptionFormat[_]] &
      !x.vs.contains(fieldName) =>
      None.asInstanceOf[T]
    case x: JObject =>
      try reader.read(x.vs(fieldName))
      catch {
